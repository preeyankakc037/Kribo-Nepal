...
const Login = ({ initialRole }) => {
  ...
  return <><Navbar /><main className="auth-page"><section className="auth-panel">
    ...
    <form className="signup-form" onSubmit={complete}>
      ...
      {step === 1 && <div className="form-step"><h2>Essential information</h2><p>Tell us how people can contact you.</p><div className="form-grid"><Field label={role === 'farmer' ? 'Full name' : 'Full name / business name'}><input required name="name" value={name} onChange={(e) => setName(e.target.value)} /></Field><Field label="Mobile number"><input required name="mobile" type="tel" placeholder="98XXXXXXXX" onChange={(e) => setDetails({ ...details, mobile: e.target.value })} /></Field><Field label={role === 'farmer' ? 'Address' : 'Business / personal address'}><input required name="address" onChange={(e) => setDetails({ ...details, address: e.target.value })} /></Field><Field label="Email ID" optional><input name="email" type="email" onChange={(e) => setDetails({ ...details, email: e.target.value })} /></Field><Field label={role === 'farmer' ? 'Farm location' : 'Operating / trading location'}><input required name="location" placeholder="Province, district, municipality" onChange={(e) => setDetails({ ...details, location: e.target.value })} /></Field></div></div>}
